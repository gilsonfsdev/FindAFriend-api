...
}, mode, concurrent, shuffle2, each, suiteOptions) {
  const tasks = [];
  const factoryQueue = [];
  let suite2;
  initSuite();
...
// node_modules/vitest/dist/vendor-index.23ac4e13.js
var _a;
...
function parseStacktrace(stack, ignore = stackIgnorePatterns) {
  const stackFrames = stack.split("\n").map((raw) => {
    ...
    if (!stack2 || ignore.length && ignore.some((p2) => stack2.file.match(p2)))
      return null;
    return stack2;
  }).filter(notNullish);
  return stackFrames;
...
// node_modules/vitest/dist/vendor-vi.dd6706cb.js
...
// node_modules/vitest/dist/vendor-run-once.1fa85ba7.js
...
// node_modules/vitest/dist/vendor-index.2af39fbb.js
...
var EditOrgUseCase = class {
  constructor(orgsRepository2) {
    this.orgsRepository = orgsRepository2;
  }
  async execute({
    id,
    president,
...
// src/use-cases/edit-org.spec.ts
var orgsRepository;
var sut;
describe("Edit Pet Use Case", () => {
  ...
  it("should be able to edit a pet", async () => {
    ...
    globalExpect(org.title).toEqual("Nova organiza\xE7\xE3o");
    globalExpect(orgsRepository.items[0]).toEqual(
